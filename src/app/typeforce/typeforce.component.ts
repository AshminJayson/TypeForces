...
export class TypeforceComponent implements AfterViewChecked {
	...
	maxwords: number = 40;
	textgen: string =
		'random text that I just got from somewhere to the changes that are here.';
	...
	constructor(
		...
		private storyGenerator: StorygptService,
	...
	@ViewChildren('charactersonscreen', { read: ElementRef })
	charactersonscreen!: QueryList<ElementRef>;
	...
	checkKeyPress(keypressed: string) {
		if (keypressed === this.charactergenarray[this.currentarrayindex])
			this.setCorrectCharacter();
		else if (keypressed === 'Backspace') this.rewindActiveCharacter();
		else if (this.currentarrayindex < this.textgen.length)
			this.setIncorrectCharacter();
	}
	...
	setActiveCharacter() {
		...
		const characterunderstudy = this.charactersonscreen.get(
			this.currentarrayindex
		);
		this.renderer.setAttribute(
			characterunderstudy?.nativeElement,
			'class',
			'active'
		);
	}
	...
	rewindActiveCharacter() {
		let characterunderstudy = this.charactersonscreen.get(
			this.currentarrayindex
		);
		this.renderer.setAttribute(
			characterunderstudy?.nativeElement,
		...
		characterunderstudy = this.charactersonscreen.get(
			this.currentarrayindex
		);
		if (
			characterunderstudy?.nativeElement.getAttribute('class') ===
			'correct'
		)
	...
	reportScore() {
		this.status = 2;
		this.wpm = Math.round(
			this.correctcharacters / (5 * this.stopwatch.getTimeMinutes())
		);
		this.accuracy = Math.round(
			(this.correctcharacters / this.textgen.length) * 100
		);

		if (this.currentarrayindex === this.textgen.length) {
			this.firestoreservice.addScore(
				this.wpm,
				this.accuracy,
				this.elapsedtime
			);
	...
	setStoryType(event: MouseEvent) {
		let storytypeselectors = document.querySelectorAll(
			'.story-type-selector'
		);
	...
	ngAfterViewChecked(): void {
		if (this.charactersonscreen.get(0) && !this.viewcheckedstate) {
			...
			this.renderer.setAttribute(
				this.charactersonscreen.get(0)?.nativeElement,
				'class',
				'active'
			);
